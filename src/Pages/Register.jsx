...
export default function Sign_in() {
  ...
  useEffect(() => {
    if (inputdata?.countryCode === "Select Country (optional)") {
      setInputdata({ ...inputdata, countryCode: "" });
    }
  ...
  const HandleSubmit = async (e) => {
    ...
    if (!inputdata.fullName) {
    ...
    } else if (!chks.test(inputdata.fullName)) {
      setFormError("Please  Enter valid Name ");
    } else if (!regex.test(inputdata.email)) {
      setFormError("Please enter Valid Email ");
    } else if (!inputdata.countryCode && inputdata.mobile) {
      setFormError("Please select country code");
    } else if (inputdata.mobile && !phoneRegex.test(inputdata.mobile)) {
      setFormError("Please enter valid phone number");
    } else if (!passregex.test(inputdata.password)) {
      setFormError(
        "Password must contain 1 uppercase, 1 lowercase, 1 number, 1 special character, and should be at least 8 characters long"
      );
    } else if (inputdata.confirmPassword !== inputdata.password) {
      setFormError("Confirm password does not match Password entered");
    } else {
      setDisable(true);
      setFormError("");
      const RowName = await encryptData(inputdata.fullName);
      const RowEmail = await encryptData(inputdata.email);
      const RowCountryCode = await encryptData(inputdata.countryCode);
      const RowMobile = await encryptData(inputdata.mobile);
      const RowPassword = await encryptData(inputdata.password);
      const RowReferral = await encryptData(inputdata.referral);
      const payload = {
        fullName: RowName,
        email: RowEmail,
        mobile: RowMobile,
      ...
      try {
        const response = await axiosMain.post("/signup", payload);
  ...
  return (
    <>
      <Layout />
      <section className="signup-sec reginster-page-only">
        <Container>
          <Row>
